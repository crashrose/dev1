...
# TODO: Set method to write credentials back to .json file
  ...
  def new_g_cal_event(id)
    event = @api_client.execute(:api_method => @calendar_api.events.get,
                                          :parameters => {'eventId' => id, 'calendarId' => @g_cal_id},
                                          :authorization => @user_credentials )
    @event = GCalEvent.google_build(event.data, @g_cal_id)#self.google_build(event)
  end
